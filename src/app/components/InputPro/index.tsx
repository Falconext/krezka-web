...
const InputPro: FC<IInput> = ({
  ...
  const renderInput = () => {
    ...
    const inputProps = {
      ...baseProps,
      type,
      className: "rounded-md p-3 py-2 w-full border-[#e5e7eb] text-[#4d4d4d] text-[14px] focus:border-none disabled:bg-[#F3F4F6]",
  ...
  return (
    <div className="relative z-0 rounded-sm">
      <div className={mode === "flex" ? "flex items-center" : ""}>
        <div>
          {isLabel && <label className="text-[14px] text-[#515C6C] font-bold">{label}</label>}
        </div>
        {renderInput()}
      </div>
